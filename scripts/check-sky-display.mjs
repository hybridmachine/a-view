...
const browser=await chromium.launch({headless:true,args:['--enable-gpu','--use-gl=angle','--use-angle=metal'],...(process.env.BROWSER_EXECUTABLE?{executablePath:process.env.BROWSER_EXECUTABLE}:{})});
...
try{
  ...
  const pausedA=await page.locator('#painting').screenshot({animations:'disabled'});
  await page.waitForTimeout(600);
  const pausedB=await page.locator('#painting').screenshot({animations:'disabled'});
  ...
  const resumed=await page.locator('#painting').screenshot({animations:'disabled'});
  ...
  const clocks=await page.evaluate(async()=>{
    ...
    elapsed=9000;p.render(snapshot,client.now(),'night');const heldAction=action===JSON.stringify(p.cloudState)&&actionTime===1004.999;
