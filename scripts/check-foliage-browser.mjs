...
async function ready(page){await page.goto(`${base}/dev/sky-study.html`);await page.waitForFunction(()=>window.skyStudy?.ready);}
try{
  ...
  const poses=await page.evaluate(async()=>{
    const {Painting}=await import('/painting.js');const p=new Painting(document.createElement('canvas'),document.createElement('canvas'),{fallback:document.createElement('img')});await p.init();
  ...
  await page.reload();await page.waitForFunction(()=>window.skyStudy?.ready);
  ...
  const bad=await invalid.evaluate(async()=>{
    ...
    const p=new Painting(document.createElement('canvas'),document.createElement('canvas'),{scene,fallback:document.createElement('img')});await p.init();
  ...
  const runtimeFailure=await page.evaluate(()=>{
    const p=skyStudy.painter;p.foliageRenderer.draw=()=>{throw new Error('Injected foliage draw failure');};skyStudy.render();
    return p.ready&&!p.foliageRenderer&&p.foliageFailure==='Injected foliage draw failure'&&p.canvas.style.visibility==='visible'&&p.gl.getError()===p.gl.NO_ERROR;
  });check('Draw failure restores intact foreground before showing frame',runtimeFailure);
  await page.reload();await page.waitForFunction(()=>window.skyStudy?.ready);
