...
The legacy final delivery is recognized by its known type and `delivery-12` ID, not by prose. The three-day bird claim validates the milestone payload and its three actual completed-return records, including matching each completion ID to the action ID in its payload. The notice must have a nonempty action ID and one proof must match that action's completion at the notice time. Regressions reject a notice pointing to another real return outside the three proofs, as well as empty or null notice action IDs. Current nest state alone is never used to infer when completion occurred.
