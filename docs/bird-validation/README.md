...
Review follow-up: bird activity now clears when construction rendering is skipped at night, when debug/study views omit the bird, and while the settled bird is hidden in shelter. The added browser regression checks each transition and the return to a visible perch. Bird/cottage events and legacy deliveries each reuse a prepared insert statement per catch-up transaction; the delivery statement is created only when needed. The catch-up benchmark below was recorded after the bird/cottage insert change. Audio cleanup now detaches `onended` before stopping a voice and retires each voice once; a regression covers natural completion, cancellation, late callbacks, and cleanup errors. Syntax checks and all 77 tests passed after the final delivery-insert cleanup. The focused browser suite passed after the rendering/audio fixes, including native cancellation with time for queued ended events to dispatch. The other browser reports above are from the initial implementation.
